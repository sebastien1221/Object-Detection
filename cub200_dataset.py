...
class CUB200(Dataset):
    def __init__(self, data_path, image_size, transform, test_train=1, return_masks=False):

        # reading file names and information and combining into a dataframe
        class_list_path = os.path.join(data_path, 'CUB_200_2011/image_class_labels.txt')
        self.data_df = pd.read_csv(class_list_path, sep= ' ', names=['index', 'class'])

        data_list_path = os.path.join(data_path, 'CUB_200_2011/images.txt')
        cub200_df = pd.read_csv(data_list_path, sep= ' ', names=['index', 'file_path'])

        bbox_list_path = os.path.join(data_path, 'CUB_200_2011/bounding_boxes.txt')
        bbox_df = pd.read_csv(bbox_list_path, sep= ' ', names=['index', 'x', 'y', 'width', 'height'])

        split_path = os.path.join(data_path, 'CUB_200_2011/train_test_split.txt')
        ...
        self.image_root_dir = os.path.join(data_path, 'CUB_200_2011/images')
        self.mask_root_dir = os.path.join(data_path, 'segmentations')
    ...
    def get_bbox_list(self, data, img_size):
        bbox_array = [data['x'],
                      ...
                      data['width'],
                      data['height']]
        # ensuring bounding boxes are properly described within the frame
    ...
    def __getitem__(self, idx):
        data_series = self.data_df.iloc('index')[idx]
